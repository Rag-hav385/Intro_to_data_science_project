...
fleet1 <- read.csv('/Users/sanjayramrajasrinsanjayivasan/Desktop/fleet1.csv', na.strings = "-")
fleet2 <- read.csv('/Users/sanjayramrajasrinsanjayivasan/Desktop/fleet2.csv', na.strings = "-")
fleet3 <- read.csv('/Users/sanjayramrajasrinsanjayivasan/Desktop/fleet3.csv', na.strings = "-")
fleet4 <- read.csv('/Users/sanjayramrajasrinsanjayivasan/Desktop/fleet4.csv', na.strings = "-")
...
```
```{r}
#Pre-processing of Different Fleets

# Remove the 'X' column
fleet4 <- fleet4[, !names(fleet4) %in% "X"]

# Remove columns from fleet3
fleet3 <- fleet3[, !names(fleet3) %in% c("X.1", "X", "Time")]

# Remove columns from fleet2
fleet2 <- fleet2[, !names(fleet2) %in% c("X.1", "X", "Time")]

# Remove columns from fleet1
fleet1 <- fleet1[, !names(fleet1) %in% c("X.1", "X", "Time")]
...
#column_names_fleet4 <- names(fleet4)
column_names
```
```{r}
elements_to_remove <- c("X", "Time", "label", "X.1")
...
**Df (Degrees of Freedom):** 1 (Between groups), 32350 (Within groups)
**Sum Sq (Sum of Squares):** Between groups: 8006, Within groups: 3072923
**F value:** 84.3
**Pr(>F):** p-value is significantly less than 0.05 (given as <2e-16), indicating a statistically significant difference between groups.

This suggests that the variable "Engine_Coolant_Temperature_degreesC" significantly affects the "Air_Flow_Rate_from_Mass_Flow_Sensor_g_s".

Intake_Manifold_Absolute_Pressure_kPa, Engine_RPM_RPM, Vehicle_Speed_Sensor_km_h, Intake_Air_Temperature_degreesC, Absolute_Throttle_Position_, Ambient_Air_Temperature_degreesC, Accelerator_Pedal_Position_D_, Accelerator_Pedal_Position_E_, Engine_Coolant_Temperature_A_degreesC
For all these variables, the ANOVA tests show extremely low p-values (<2e-16), indicating significant differences between groups. These variables are also likely to have a significant impact on the target variable "Air_Flow_Rate_from_Mass_Flow_Sensor_g_s".

***Intake_Air_Temperature_A_degreesC***
The p-value (0.22) for this variable is greater than 0.05, suggesting that there isn't sufficient evidence to conclude a significant impact of "Intake_Air_Temperature_A_degreesC" on "Air_Flow_Rate_from_Mass_Flow_Sensor_g_s".
